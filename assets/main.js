...
function cleanInternalHrefValue(value) {
  ...
  return origin ? `https://rogersense.com${cleanPath}` : cleanPath;
}
...
function cleanInternalLinks(html = '') {
  const pages = {
    products: '/shop', shop: '/shop', cases: '/cases', blog: '/blog', about: '/about', quote: '/quote',
  ...
  out = out.replace(/((?:src|href)\s*=\s*["'])assets\//gi, '$1/assets/');
  out = out.replace(/(href\s*=\s*)(["'])([^"<>]*?)\2/gi, cleanInternalHrefAttr);
  out = out.replace(/\s+on[a-z][\w:-]*\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '');
